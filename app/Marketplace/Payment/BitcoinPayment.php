...
//            $accountBalance = $this -> bitcoind -> getbalance($params['account'], (int)config('marketplace.bitcoin.minconfirmations'));
    ...
    {
	var_dump("Bitcoin SendToAddress Function");die();
...
//        }
        ...
        if ($this->bitcoind->error) {
		var_dump("------- Bitcoin sendToMany Function Error----------</br>");
        	echo($this->bitcoind->error);
    ...
    {
        return round( BitcoinConverter::btcToUsd($coin), 2, PHP_ROUND_HALF_DOWN );
    }
